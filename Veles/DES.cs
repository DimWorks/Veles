...
namespace Veles
{
    class DES
    {
        ...
        private const int sizeOfChar = 16;   //размер одного символа в юникоде 2 байта (16 бит)
        ...
        public Tuple<string, string> Encrypt(string message, string key)
        {
            ...
            message = MessageToRightLength(message);
            ...
            string decryptKey = KeyToPrevRound(key);

            //Output call_output = new Output();
            //call_output.Text = "DES";
            //call_output.hat.Text = "Сгенерированный ключ расшифровки(скопируйте):";
            //call_output.Sometext = StringFromBinaryToNormalFormat(decryptKey);
            //if(Application.OpenForms["Output"] == null)
            //{
            //    call_output.Show();
            //}
            ...
            Tuple<string, string> result = new Tuple<string, string>(StringFromBinaryToNormalFormat(output), StringFromBinaryToNormalFormat(decryptKey));
            return result;
            //return StringFromBinaryToNormalFormat(output);
        }
        ...
        public string Decrypt(string key, string message)
        {
            ...
            output = StringFromBinaryToNormalFormat(output);
            output = output.Trim(new char[] { '#' });
            return output;
        }

        //доводим строку до размера, чтобы делилась на sizeOfBlock
        private string MessageToRightLength(string input)
        {
        ...
        private void MessageToBlocks(string input)
        {
